...
public class HITGvyPermissionDPH extends GroovyInjectionBase {
	...
	public String execute(GroovyEvent event) {
		...
		if(UnitNbr != null && !UnitNbr.isEmpty()){
			...
			String query = "SELECT TOP 1 [Action],[Nota] FROM [HIT_SQL01].[N4EDI].[dbo].[TransaccionWebApiHPUDPH] WHERE [UnitNbr] ='" + UnitNbr +"' AND Estatus ='PENDIENTE' ORDER BY Id DESC";
			
			logInfo("[HITGvyPermissionDPH]: " + query);
			...
			if(Lista.size() > 0){
				...
				if(rows.length > 0){
					   ...
					   xmlQuery = "<hpu>"+
								...
								"</entities>" +
								"<flags>" +
								"<flag hold-perm-id=\"DPH_PERMISO\" action=\"" + Action + "\" note=\"" + Note +"\"/>" +
								"</flags>" +
								"</hpu>";
					   ...
					   String sql = "UPDATE [HIT_SQL01].[N4EDI].[dbo].[TransaccionWebApiHPUDPH] SET [Estatus] = 'COMPLETADO',[FechaActualizacion] = GETDATE() WHERE Id = (SELECT MAX(Id) FROM [HIT_SQL01].[N4EDI].[dbo].[TransaccionWebApiHPUDPH] WHERE [UnitNbr] ='" + UnitNbr + "') AND Estatus ='PENDIENTE';";
					   									
					   logInfo("[HITGvyPermissionDPH]: " + sql);
																		
					   if (commonResponse.getStatus().equals(ERRORS)) {
						   logInfo("[HITGvyPermissionDPH]: Web service returned error: " + commonResponse.getStatusDescription());					   	 
					   }else{ 						  
